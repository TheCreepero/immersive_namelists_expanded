<#
.SYNOPSIS
    Test runner script for Immersive Namelists Expanded (INEX).

.DESCRIPTION
    Executes Pester test suites for namelists, documentation synchronization,
    and build automation. Supports CI export to NUnit XML format.

.PARAMETER Suite
    Filter test execution to a specific suite: 'All' (default), 'Namelists', 'Documentation', or 'BuildScript'.
...
.EXAMPLE
    powershell -File .\tests\Run-Tests.ps1 -Suite Namelists
...
[CmdletBinding()]
param(
    [ValidateSet('All', 'Namelists', 'Documentation', 'BuildScript')]
    [string]$Suite = 'All',
...
if ($Suite -eq 'All' -or $Suite -eq 'Namelists') {
    $testFiles.Add((Join-Path $testDir "Namelists.Tests.ps1"))
}
...
Write-Host "  INEX Test Suite Runner (Pester $($pesterModule.Version))" -ForegroundColor Cyan
